...
export class FirebaseUtils {
    static initFirebase(FIRE_BASE_CONFIG) {
    ...
    static async addUpdateCommand(refKey, value) {
        if (isEmpty(refKey))
            return;
        if (value === undefined)
            value = null;
...
const getUserRefKeyOnFirebase = function () {
    if (UserUtils.getUserObj().id)
        return UserUtils.getUserObj().id.hashCode();
    return "{0}";
};
